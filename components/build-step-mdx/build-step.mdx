...
import { SYSTEM_ENVS } from '~/lib/data/system-env-variables'
...
<Note>
  ...
  <Link href="/docs/serverless-functions/introduction">
...
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Production**  | When selected, the Environment Variable will be applied to your next Production Deployment. To create a Production Deployment, push a commit to the [Production Branch](/docs/git-integrations#production-branch) or run `vercel --prod`. |
...
<Note>
  ...
  <Link href="/docs/platform/deployments#deduplication">deduplication</Link>{' '}
...
The <ProductName /> platform reserves the usage of some Environment Variable names by default. You can find a list of these names in the [limits documentation](/docs/platform/limits/#reserved-variables).

### Using a `.env` File

We generally recommend only adding your Environment Variables in the [Project Settings](#environment-variables), as that's the only way to achieve different Environment Variables based on the Environment (Development, Preview or Production).
...
Deploying Git submodules with the [Git Integration](/docs/git-integrations) is supported as long as the submodule is publicly accessible via the HTTP protocol.
Git submodules that are private or requested over SSH will fail during the Build step.
...
<Card title="Git Integration" href="/docs/git-integrations">
  Learn more about how <ProductName /> integrates with Git to make your workflow
  easier.
</Card>

<Card title="Deploy Hooks" href="/docs/advanced/deploy-hooks">
