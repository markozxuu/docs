...
| `res.redirect(statusCodeOrlUrl, [url])`                                                    | A function to set a redirect, redirects to the URL derived from the specified path, with specified status, a positive integer that corresponds to an [HTTP status code](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes). If not specified, status defaults to “307“ Temporary Redirect ”. | Response |
