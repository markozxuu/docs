...
export const meta = {
  ...
  lastEdited: '2020-08-18T18:29:32.000Z',
}
...
<OrgName /> works with multiple cloud partners in order to provide services to
end users.
...
When using Next.js, only `memory` and `maxDuration` can be configured within the [functions](https://vercel.com/docs/configuration#project/functions) property. Next.js automatically handles the other configuration values for you.
...
If you would like to increase this limit, please [contact Vercel Support](/support/request).

## Bypassing Password Protection Programmatically

When [Password Protection](/docs/v2/platform/projects#password-protection) is enabled on your Project,
